...
class SettingScreenScreenshotTests {
    ...
    private val successUiState = SettingUiState.Success(
        ...
        dynamicColor = false,
        monthStartDay = 1,
        creditCardReminderEnable = false,
        reimbursementReminderEnable = false,
        autoRecordEnable = false,
        autoRecordMatchTexts = emptyList(),
    )
    ...
    @Test
    fun settingScreen_autoRecord_multipleThemes() {
        composeTestRule.captureMultiTheme(
            name = "SettingScreen",
            overrideFileName = "SettingScreen_auto_record",
        ) {
            SettingScreen(
                supportFingerprint = false,
                uiState = successUiState.copy(
                    autoRecordEnable = true,
                    autoRecordMatchTexts = listOf("微信支付 * 元", "支付宝 * 元"),
                ),
                shouldDisplayBookmark = "",
                onRequestDismissBookmark = {},
                dialogState = DialogState.Dismiss,
